...
def RunModel(df, y, home_team, away_team, year, month, B365CH, B365CD, B365CA, B365C_OVER, B365C_UNDER):
    
    team_data = pd.get_dummies(df[["HomeTeam", "AwayTeam"]])
    team_names = team_data.columns
    
    other = df[["Year", "Month", "B365CH","B365CD","B365CA","B365C>2.5","B365C<2.5"]]
...
def GetHomeTeamShots(df, home_team, away_team, year, month, 
                     B365CH, B365CD, B365CA, B365C_OVER, B365C_UNDER):
    if (0 == df.isnull().sum().sum()):
        y = df["HS"] # Home Team Shots.
        return RunModel(df=df, y=y, home_team=home_team, away_team=away_team, year=year, month=month, 
